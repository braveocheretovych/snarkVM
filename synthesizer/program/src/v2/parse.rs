    ...
    for ProgramCoreV2<N, Instruction, Command>
{
    ...
    fn parse(string: &str) -> ParserResult<Self> {
        ...
        // Parse the constructor if it exists.
        let (string, constructor) = opt(ConstructorCore::parse)(string)?;
        // Parse the whitespace and comments from the string.
        let (string, _) = Sanitizer::parse(string)?;
        ...
        let mut program = match ProgramCoreV2::<N, Instruction, Command>::new(id) {
            Ok(program) => program,
            Err(error) => {
                eprintln!("{error}");
        ...
        // Add the constructor if it exists.
        if let Some(constructor) = constructor {
            match program.add_constructor(constructor) {
                Ok(_) => (),
                Err(error) => {
                    eprintln!("{error}");
                    return map_res(take(0usize), Err)(string);
        ...
        // Add the metadata to the program.
        for metadata in metadata {
            match program.add_metadata(metadata) {
                Ok(_) => (),
                Err(error) => {
                    eprintln!("{error}");
                    return map_res(take(0usize), Err)(string);
...
function compute:
    ...
    fn test_program_display() -> Result<()> {
        let expected = r"program$2 to_parse.aleo;
